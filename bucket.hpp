...
public:
    ...
    /* make sure there is only one root, a bit like a singelton*/
    static shared_ptr<Bucket> createRoot(int r){
        if (root == nullptr) {
            ...
            srand(r*42); /* set different random seed for each processor */
    ...
    static shared_ptr<Bucket> root; /* one root per processor */
    static shared_ptr<Bucket> bb; /*boundary bucket */


    /*** public methods ***/
    shared_ptr<Bucket> operator() (int i, int j) const;
    inline void printNeighbours() const;
    inline bool isBnd( ) {return is_bnd;}; /* don't just return false, because of downcasting */ 
...
protected:
    Bucket() {}; /* for boundary bucket, TODO: make that nicer, not safe ? */

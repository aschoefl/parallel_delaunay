...
int Bucket::calculateDelauney(int step){ 
    ...
    if (root->r() != r()) throw runtime_error("calculate Delauney called from wrong processor");
    ...
    if (step == 0){ 
        cout << root->r() << ": start calculate delauney for point " << poly.c << " in bucket (" <<i() << ", " << j() << ") "  << endl;
        it = 0;}
    while (it <poly.points.size()) {
        ...
        if (poly.V.empty()) {
            ...
            Point v = poly.V.back();
            Point a = (poly.c+v)/2;
            Point b = a + Point((v-poly.c).y, (a-poly.c).x);
            ...
            cout << endl << root->r()  << "." << it 
            << " of "<< poly.points.size() << " with v = " << v <<", center = " << poly.c << endl << poly << endl;

            // cout << root->r() << "." << it << ": last = " << last << " first = " << first << endl << endl; 
            ...
            for (int k=old_size-1; k>=0; k--) {
                if (!inHv(poly.voronoi[k])) {
                    cout << r() << ": add to erase "  <<k << endl;
                    to_erase.push_back(k);
                }
            }

            /* sort in reverse order */
            sort(to_erase.begin(), to_erase.end(), greater <>());

            for (int l = 0; l<to_erase.size(); l++) {
                int k = to_erase[l];
                if (k == 0) {
                    /* first and last voronoi point are to erase */
                    if (to_erase.front() == old_size-1) {
                        poly.points.erase(poly.points.begin()+k);
                        // cout << "here" << endl;
                    }
                } else if(k-1 == to_erase[l+1]){
                    poly.points.erase(poly.points.begin()+k);
                }
            ...
            if (index == 0) int tmp =0;//it = 0;
            else it = index-1;

            poly.calculateVoronoi();
            cout  << root->r()  << "." << it 
            << " after calc Voro "<<  poly << endl;
    ...
    if (MAX_PNTS <10 || N < 1000) {
        auto tmp = find(points.begin(), points.end(), static_cast<Point>(poly.c));
        int index = distance(points.begin(), tmp);
        poly.printPoints(to_string(i()*10000+j()*10+index));
    } else throw runtime_error("too many buckets or points per bucket for that storage method");
    return 0;
}
